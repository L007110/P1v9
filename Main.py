...
def rl(mean_loss_across_epochs=None, gnn_optimizer=None):
    ...
    while True:
        ...
        # ==================================================================
        if USE_GNN_ENHANCEMENT and graph_data_t_plus_1 is not None:
            try:
                global_gnn_model.eval()
                graph_data_t_plus_1_dev = move_graph_to_device(graph_data_t_plus_1, device)
                with torch.no_grad():
                    all_q_values_t_plus_1_online = global_gnn_model(graph_data_t_plus_1_dev)
                global_gnn_model.train()
            except Exception as e:
                debug(f"GNN S_t+1 forward pass for action selection failed: {e}")
                all_q_values_t_plus_1_online = None
        ...
        for dqn in global_dqn_list:
            ...
            if dqn.vehicle_exist_curr:
                ...
                if USE_GNN_ENHANCEMENT and all_q_values_t_plus_1_online is not None:
                    actions_tensor = all_q_values_t_plus_1_online[dqn.dqn_id - 1]
            ...
            for i_episode in range(TEST_EPISODES_PER_COUNT):
                ...
                if is_gnn_model:
                    try:
                        ...
                        end_time = time.time()
                        # GNN 模式: 记录一次全局推理时间
                        step_decision_times.append((end_time - start_time) * 1000.0)
                    except Exception as e:
